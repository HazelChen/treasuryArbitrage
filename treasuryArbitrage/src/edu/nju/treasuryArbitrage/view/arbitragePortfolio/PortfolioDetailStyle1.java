...
public class PortfolioDetailStyle1 extends PortfolioDetail {
	...
	private void init() {
		for (int i = 0; i < 2; i++) {
			...
			title[i].setFont(new Font("微软雅黑", Font.PLAIN, 22));
			this.add(title[i]);
	...
	private void initLeft(int index) {
		...
		labels[0] = new JLabel("序号");
		labels[1] = new JLabel("涨跌幅");
	...
	private void initRight(int index) {
		JLabel[] labels = new JLabel[2];
		labels[0] = new JLabel("现价");
		labels[1] = new JLabel("涨跌");

		for (int i = 0; i < labels.length; i++) {
			labels[i].setBounds(width / 2 + 3, 33 + (LABEL_HEIGHT + 3) * i + 100 * index,
					width, LABEL_HEIGHT);
			labels[i].setFont(new Font("微软雅黑", Font.PLAIN, 18));
			labels[i].setForeground(Color.WHITE);
			this.add(labels[i]);
		}
