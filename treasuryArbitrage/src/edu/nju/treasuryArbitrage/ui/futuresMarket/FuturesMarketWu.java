...
public class FuturesMarketWu extends JPanel implements ComponentPanel {
	...
	private JTable futuresTable1;
	private JTable futuresTable2;
	private JTable futuresTable3;
	private JTable futuresHeader;
	private FuturesPanel detailPanel;
	private TableCellTextPaneRenderer tctpHeader;
	private TableCellTextPaneRenderer tctpTable1;
	private TableCellTextPaneRenderer tctpTable2;
	private TableCellTextPaneRenderer tctpTable3;
	private BlueTableCellTextPaneRenderer btctpTable1, btctpTable2,
			btctpTable3;
	private RedTableCellTextPaneRenderer rtctpTable1, rtctpTable2, rtctpTable3;
	private GreenTableCellTextPaneRenderer gtctpTable1, gtctpTable2,
			gtctpTable3;
	private Arb_detail arb1, arb2, arb3;
	private Object[][] tableInfo1, tableInfo2, tableInfo3;
	private DefaultTableModel model1, model2, model3;
	private LineChart chart1, chart2, chart3;

	public FuturesMarketWu() {
		...
		futuresTable1
				.setBounds(0, 10 + HEADER_HEIGHT, WIDTH - 20, TABLE_HEIGHT);
		futuresTable2.setBounds(0, 10 + HEADER_HEIGHT + TABLE_HEIGHT,
				WIDTH - 20, TABLE_HEIGHT);
		futuresTable3.setBounds(0, 10 + HEADER_HEIGHT + 2 * TABLE_HEIGHT,
				WIDTH - 20, TABLE_HEIGHT);
	...
	public void update(ArrayList<Arb_detail> arb_list) {
		...
		String[] columnTitle = { "代码", "交割月份", "现价", "涨跌", "涨跌幅", "买量", "买价",
				"卖价", "卖量", "成交量", "持仓量", "日增仓", "前结算价", "今开", "最高", "最低", "时间" };
		...
		if (arb1.getPresentPrice() > arb1.getPreSettlePrice()) {
			futuresTable1.getColumn(columnTitle[2])
					.setCellRenderer(rtctpTable1);
		} else if (arb1.getPresentPrice() < arb1.getPreSettlePrice()) {
			futuresTable1.getColumn(columnTitle[2])
					.setCellRenderer(gtctpTable1);
		} else {
			futuresTable1.getColumn(columnTitle[2])
					.setCellRenderer(btctpTable1);
		}
		if (arb1.getChange() > 0) {
			futuresTable1.getColumn(columnTitle[4])
					.setCellRenderer(rtctpTable1);
		...
		if (arb2.getPresentPrice() > arb2.getPreSettlePrice()) {
			futuresTable2.getColumn(columnTitle[2])
					.setCellRenderer(rtctpTable2);
		} else if (arb2.getPresentPrice() < arb2.getPreSettlePrice()) {
			futuresTable2.getColumn(columnTitle[2])
					.setCellRenderer(gtctpTable2);
		} else {
			futuresTable2.getColumn(columnTitle[2])
					.setCellRenderer(btctpTable2);
		}
		if (arb2.getChange() > 0) {
			futuresTable2.getColumn(columnTitle[4])
					.setCellRenderer(rtctpTable2);
		...
		if (arb3.getPresentPrice() > arb3.getPreSettlePrice()) {
			futuresTable3.getColumn(columnTitle[2])
					.setCellRenderer(rtctpTable3);
		} else if (arb3.getPresentPrice() < arb3.getPreSettlePrice()) {
			futuresTable3.getColumn(columnTitle[2])
					.setCellRenderer(gtctpTable3);
		} else {
			futuresTable3.getColumn(columnTitle[2])
					.setCellRenderer(btctpTable3);
		}
		if (arb3.getChange() > 0) {
			futuresTable3.getColumn(columnTitle[4])
					.setCellRenderer(rtctpTable3);
		...
		for (int i = 5; i < 16; i++) {
			futuresTable1.getColumn(columnTitle[i])
					.setCellRenderer(btctpTable1);
			futuresTable2.getColumn(columnTitle[i])
					.setCellRenderer(btctpTable2);
			futuresTable3.getColumn(columnTitle[i])
					.setCellRenderer(btctpTable3);
	...
	private JTable getHeader() {
		String[] header = { "代码", "交割月份", "现价", "涨跌", "涨跌幅", "买量", "买价", "卖价",
				"卖量", "成交量", "持仓量", "日增仓", "前结算价", "今开", "最高", "最低", "时间" };
		String[][] headerInfo = { new String[] { "代码", "交割月份", "现价", "涨跌",
				"涨跌幅", "买量", "买价", "卖价", "卖量", "成交量", "持仓量", "日增仓", "前结算价",
				"今开", "最高", "最低", "时间" } };
	...
	private JTable getTable1() {
		...
		Object[][] futuresInfo = { new Object[] { "TF1412", "2014年12月",
				...
				arb1.getRepository(), arb1.getDailyWarehouse(),
				arb1.getPreSettlePrice(), arb1.getOpen(), arb1.getHigh(),
				arb1.getLow(), hour + ":" + min } };
	...
	private JTable getTable2() {
		...
		Object[][] futuresInfo = { new Object[] { "TF1503", "2015年03月",
				...
				arb2.getRepository(), arb2.getDailyWarehouse(),
				arb2.getPreSettlePrice(), arb2.getOpen(), arb2.getHigh(),
				arb2.getLow(), hour + ":" + min } };
	...
	private JTable getTable3() {
		...
		Object[][] futuresInfo = { new Object[] { "TF1506", "2015年06月",
				...
				arb3.getRepository(), arb3.getDailyWarehouse(),
				arb3.getPreSettlePrice(), arb3.getOpen(), arb3.getHigh(),
				arb3.getLow(), hour + ":" + min } };
